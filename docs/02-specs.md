...
결함이 있어 시세가 조용히 꺼졌다(→ 워밍업·국면 분류 중단). 연결 상태는 **감사 로그(`MARKET`)로
가시화**하되 **up↔down 전환 시에만 1건 기록**한다(30초마다 반복되는 재연결 실패를 매번 남기지
않음 — 로그 폭주 방지). `MarketDataService.running`은 이 태스크가 살아있는지로 판정하므로,
무한 재연결 동안 `market_running`은 `true`를 유지한다.

> 핸드셰이크 거부(`did not receive a valid HTTP response`)가 반복되면 KIS **실시간 웹소켓 세션
> 제한**(계정당 동시 1세션)일 수 있다. 잦은 재시작으로 이전 세션이 KIS 측에 남으면 새 연결이
> 거부되며, 보통 시간이 지나거나 장 마감 후 풀린다. → 운영 중 잦은 재시작을 피한다.
