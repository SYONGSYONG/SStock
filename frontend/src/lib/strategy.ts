...
export const STRATEGY_HELP: Record<string, StrategyHelp> = {
  ma_cross: {
    ...
    examples: [
      "상황①(매수): 50틱봉에서 주가가 6,000원까지 밀렸다가 반등해 MA5가 MA20을 아래→위로 뚫고 2봉(확인봉) 유지 → 골든크로스 매수. 단, 이격이 1틱 미만으로 살짝만 스치면 무시합니다.",
      "상황②(매도): 보유 중 주가가 꺾여 MA5가 MA20 아래로 내려가 2봉 유지 → 데드크로스 매도. 추세MA(예: MA100) 아래로 떨어지면 더 빨리 청산합니다.",
  ...
  rsi_ma: {
    ...
    examples: [
      "상황①(매수): 주가가 추세MA(50틱봉 MA50) 위에 있는 상승추세에서, 잠깐 눌리며 RSI가 30 아래로 갔다가 30 위로 회복 → 눌림목 매수. 단, 현재가가 MA보다 너무 높이(추격거리 초과) 떠 있으면 늦은 추격으로 보고 보류합니다.",
      "상황②(매도): 보유 중 RSI가 70 위로 과열됐다가 70 아래로 꺾이면 → 과열 청산. 또는 현재가가 MA−2틱 아래로 2봉 유지되면 추세 이탈로 청산합니다.",
      "상황③(노이즈 보류): 현재가가 MA 바로 위아래 ±2틱 안에서 오락가락하면 → 추세 판단을 보류해 잦은 진입을 막습니다.",
    ],
    note: "‘틱봉’ 단위로 계산합니다. 틱봉이 클수록(50~100) 노이즈가 줄어 안정적이지만, 워밍업이 길어집니다 — 예: 50틱봉×MA50이면 첫 신호까지 약 2,550틱이 필요하고 봇 재시작 시 다시 누적합니다. 추세 MA는 느릴수록 필터가 잘 동작합니다(너무 짧으면 눌림에 현재가가 MA 아래로 내려가 매수가 막힘).",
  },
};
